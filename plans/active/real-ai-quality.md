...
- [x] 2026-08-15 05:21Z — Add and pass API provider, retry, cancellation, prompt-injection, cache, migration, no-call-before-consent, verified-grounding, and no-persistence-on-verification-failure tests.
...
- [x] 2026-08-15 05:52Z — Resolve every owner-review implementation finding: isolate provider secrets from Electron, make revocation race-safe, guard cache dimensions, preflight oversized evidence, correct negative evaluation behavior, include prompt versions, and gate the live evaluation before key inspection or network access.
- [x] 2026-08-15 05:54Z — Rerun the complete root formatter, linter, strict type checks, 2 launcher tests, 3 contract tests, 12 desktop tests, 35 API tests, native Electron/API smoke, and offline 16-case fixture validation; all pass. Confirm the legal hold exits with code 2 before a provider request.
- [x] 2026-08-15 05:57Z — Complete a narrow read-only re-review of the remediated paths; no remaining material finding is supported.
...
- 2026-08-14 — The original development launcher copied one parent environment to both child processes, which exposed `NEBIUS_API_KEY` to Electron main/preload. The launcher now builds separate child environments: the API retains server configuration while Electron receives only an execution/cache/locale allowlist and the generated loopback API capability.
- 2026-08-14 — A policy check only at answer start was insufficient: revocation could race an awaited provider call and allow stale cache or answer writes. Active project runs are now canceled on revocation, provider stages recheck the original revision, and cache/final-answer writes validate that revision atomically.
- 2026-08-14 — Embedding dimensions must participate in cache validity even when provider, model, evidence ID, and content hash are unchanged. Cache lookup now filters the configured dimension and re-embeds mismatches.
- 2026-08-14 — Imported Markdown sections and PDF pages may be larger than the provider's per-input character limit even when the source file is accepted. Remote answering now returns a typed, no-provider-call failure before indexing; provenance-preserving chunking remains follow-up work.

## Outcome and follow-ups

The optional Nebius implementation is code-complete for deterministic and mocked acceptance. Remote processing remains project-opt-in, secrets stay API-only, revocation cancels active work and prevents stale persistence, retrieval and two-pass verification preserve structured provenance, and classified failures remain visible without fake fallback. The complete repository checks pass: formatter, lint, strict TypeScript/Python typing, 2 launcher tests, 3 contract tests, 12 desktop tests, 35 API tests, the native Electron-to-real-local-API smoke, and offline validation of all 16 synthetic evaluation cases. The final narrow owner re-review found no remaining material issue. The only test warning is the existing Starlette notice that `TestClient` will eventually prefer `httpx2`; the direct runtime provider adapter intentionally remains on the smaller locked `httpx` dependency for this milestone.

No live Nebius request was made and no benchmark result is claimed. This plan remains active because completion still requires written confirmation that the thresholded evaluation is permitted, a locally configured key, the legal-gated live evaluation, and the manual two-source real-provider acceptance path. The pre-existing untracked `examples/EGO_sEMG.md` remained untouched and unstaged.
